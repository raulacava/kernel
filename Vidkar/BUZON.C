...
{	      
   ...
   /* // inicializa_tarea(&letreros,4,1);//se asigna ID 4 y prioridad 3 a tarea letreros
	...
	inicializa_tarea(&generador,1,1);//se asigna ID 4 y prioridad 1 a tarea letreros
    inicializa_tarea(&consumidor,2,1); //se asigna ID 1 y prioridad 1 a tarea pelota
	inicializa_tarea(&consumidor,3,1);//se asigna ID 2 y prioridad 1 a tarea editor
	inicializa_tarea(&consumidor,4,1);//se asigna ID 3 y prioridad 1 a tarea reloj
	inicializa_tarea(&idle,5,2);//se asigna ID 5 y prioridad 2 a tarea letreros
...
void letreros(void)
{
 ...
 void consumidor(void)
 {
 	  ...
 	  while(1)
 	  {
 	  	  ...
 	  	  switch(PtrRun->Este)
		    {
		    	case 2://coordenadas de la pantalla del proceso de id 1 -pelota-
		    		gotoxy(60,5);			
		    	break;
		    	case 3://coordenadas de la pantalla del proceso de id 2 -editor-
		    		gotoxy(20,17);
		    	break;
		    	case 4://coordenadas de la pantalla del proceso de id 3 -reloj-
		    		gotoxy(60,17);
...
void generador(void)
{
	...
	while(1)
	{
		   ...
		   disable();
		   gotoxy(20,5);
		   printf("%s enviado...",mensaje[aux]);
		   enable();
		   //signal(&s);
		   send(mensaje[aux]);
		   Retrasa(PtrRun->Este,40);
